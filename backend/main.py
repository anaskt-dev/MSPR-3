...
# Importe le module `database` qui contient la logique d'initialisation de
# la base de données.
import database

# Importe le module 'models' pour s'assurer que les modèles de base de
# données sont reconnus par SQLAlchemy avant l'initialisation de la base
# de données.
# Importe les routeurs définis dans `routes.py` pour les endpoints de
# l'API (données, prédiction).
...
# Définit un événement de démarrage qui sera exécuté au lancement de
# l'application.
...
def import_initial_data():
    ...
    try:
        # Vérifie si des données existent déjà en utilisant text() pour la
        # requête SQL brute
...
# Toutes les routes définies dans `routes.py` seront accessibles via
# /api/your-endpoint.
app.include_router(api_router, prefix="/api")
# Inclut le routeur d'authentification sous le même préfixe '/api'.
# Les routes d'authentification (ex: /api/login, /api/register) seront
# accessibles.
