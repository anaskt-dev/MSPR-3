...
# ATTENTION: En production, cette clé secrète doit
# être une valeur forte et chargée depuis une variable
# d'environnement ou un service de gestion des secrets.
# Ne jamais la laisser en dur dans le code source pour
...
def verify_password(plain_password, hashed_password):
    """Vérifie si un mot de passe
    en clair correspond à un mot de passe haché."""
...
def get_user(db: Session, username: str):
    """Récupère un utilisateur de la base
    de données par son nom d'utilisateur."""
...
def authenticate_user(db: Session, username: str, password: str):
    ...
    Retourne l'objet utilisateur si
    l'authentification réussit, False sinon."""
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    """Crée un jeton d'accès JWT avec les données
    fournies et une date d'expiration."""
    to_encode = data.copy()
    if expires_delta:
...
async def get_current_user(token: str = Depends(
        oauth2_scheme), db: Session = Depends(database.get_db)):
    """Récupère et valide l'utilisateur courant à
    partir du jeton d'authentification fourni."""
    ...
    try:
        ...
        # Décode le jeton JWT en utilisant
        # la clé secrète et l'algorithme spécifié.
    ...
    # Tente de récupérer l'utilisateur de la base de données
    # en utilisant le nom d'utilisateur extrait.
...
async def login_for_access_token(
    ...
    """Endpoint pour la connexion des utilisateurs et la
    génération d'un jeton d'accès JWT."""
